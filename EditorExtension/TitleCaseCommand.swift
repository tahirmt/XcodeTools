...
class TitleCaseCommand: NSObject, XCSourceEditorCommand {
    func perform(with invocation: XCSourceEditorCommandInvocation, completionHandler: @escaping (Error?) -> Void ) -> Void {
        ...
        selections.forEach { selection in
            ...
            if let line = invocation.buffer.lines.object(at: lineNumber) as? NSString,
                ...
                invocation.buffer.lines[lineNumber] = line.replacingCharacters(in: range, with: selectedText.upperCamelCased)
            }
        }
    }
}
